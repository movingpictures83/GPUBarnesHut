...
/******************************************************************************/
void GPUBarnesHutPlugin::input(std::string file) {
 inputfile = file;
 readParameterFile(file);
}

void GPUBarnesHutPlugin::run() {}

void GPUBarnesHutPlugin::output(std::string file) {
  ...
  const int dev = atoi(myParameters["device"].c_str());
...
//  fprintf(stderr, "blocks = %d\n", blocks);
  ...
  for (run = 0; run < 1; run++) {
    ...
    nbodies = atoi(myParameters["nbodies"].c_str());
    if (nbodies < 1) {
    ...
    timesteps = atoi(myParameters["timesteps"].c_str());
...
PluginProxy<GPUBarnesHutPlugin> GPUBarnesHutPluginProxy = PluginProxy<GPUBarnesHutPlugin>("GPUBarnesHut", PluginManager::getInstance());
